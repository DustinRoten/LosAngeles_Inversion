...
for(i in 1:(length(inversion.breaks) - 1)) {
  ...
  #make lambda_p
  lambda_p <- as.vector(priors[,2])
  
  #make S_p
  S_p <- matrix(0, nrow = nrow(priors), ncol = nrow(priors))
  diag(S_p) <- priors[,3]
  ...
  #make K
  K <- as.matrix(time.sub.xco2_df[,gsub(' ', '', priors[,1])])
  
  #make z
  z <-
    (time.sub.xco2_df$xco2 - time.sub.xco2_df$bio) -
    (time.sub.xco2_df$OCO3.bkg)
  ...
  errors <- data.frame(prior.errors = term.3,
                       ...
                       period = i)
  lambdas <- data.frame(category = priors[,1],
                        lambda = lambda_hat,
                        period = i)
  S_errors <- data.frame(category = priors[,1],
                         S = diag(S_error),
                         period = i)
  if(i == 1) {
    errors_df <- errors
