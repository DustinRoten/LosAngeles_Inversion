get_vulcan_emiss_hourly <- function(vulcan_emiss = NULL, ref.raster = NA, category = NA,
                                    sector_df = NA, date.time = NA, loc = 'US', nc.extent = NA,
  ...
  if(is.na(category)) {
    ...
    ####################################
    #find the row with the emissions of interest
    selected.category <- category
    sector.list <- subset(sector_df, category == selected.category)
    sector.list <- gsub(' ', '|', paste(sector.list[,2]))
    
    #remove the leading '|', if applicable
    if(substr(sector.list, 1, 1) == '|')
      sector.list <- substr(sector.list, 2, nchar(sector.list))
    
    #get all rasters of appropriate sectors
    all.rasters <- grep(list.files(vulcan_emiss, full.names = TRUE),
                        pattern = sector.list, value = TRUE)
