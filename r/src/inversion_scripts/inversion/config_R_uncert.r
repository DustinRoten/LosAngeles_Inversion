...
# define the overall matrix containing all the details of component-level errors
R_arr <- array(NA, dim = c(0, 5))
colnames(R_arr) <-
  c("component", "RMSE", "correlation_scale", "non_decaying?", "correlate_btwn_days?")

# R error from external csv file (added by D. Roten 06/2021)
R_errors <- read.csv('included.errors.csv')

# ~~~~~~~~~~~~~~~ R_part ~~~~~~~~~~~~~~~ #
# uncertainty in STILT based on the finite number of particles released
rmse_part <- subset(R_errors, name == 'R_part')$RMSE
if(length(rmse_part) == 0) {rmse_part <- 0}
...
rmse_aggr <- subset(R_errors, name == 'R_aggr')$RMSE
if(length(rmse_aggr) == 0) {rmse_aggr <- 0}
Lt_aggr <- NA #correlation time scale, in days
correlate_full_aggr <- F #if TRUE, apply RMSE to off_diagonals without decay
...
rmse_eddy <- subset(R_errors, name == 'R_eddy')$RMSE
if(length(rmse_eddy) == 0) {rmse_eddy <- 0}
...
rmse_bkgd <- subset(R_errors, name == 'R_bkgd')$RMSE
if(length(rmse_bkgd) == 0) {rmse_bkgd <- 0}
Lt_bkgd <- NA #correlation time scale, in days
correlate_full_bkgd <- F #if TRUE, apply RMSE to off_diagonals without decay
...
rmse_transPBL <- subset(R_errors, name == 'R_PBL')$RMSE
if(length(rmse_transPBL) == 0) {rmse_transPBL <- 0}
#rmse_transPBL <- 0.35 * mean(z)
Lt_transPBL <- NA #correlation time scale, in days
correlate_full_transPBL <- F #if TRUE, apply RMSE to off_diagonals without decay
...
rmse_transWIND <- subset(R_errors, name == 'R_wind')$RMSE
if(length(rmse_transWIND) == 0) {rmse_transWIND <- 0}
#rmse_transWIND <- 0.40 * mean(z)
Lt_transWIND <- 3/24 #correlation time scale, in days
...
rmse_instr <- subset(R_errors, name == 'R_inst')$RMSE
if(length(rmse_instr) == 0) {rmse_instr <- 0}
...
if (include_bio){
    rmse_bio <- subset(R_errors, name == 'R_bio')$RMSE
    if(length(rmse_bio) == 0) {rmse_bio <- 0}
...
extra_sites_err_rmse <- array(0.0, dim = c(1, nsites))
colnames(extra_sites_err_rmse) <- sites

# example:
#extra_sites_err_rmse[1,"MSA"] <- 6
