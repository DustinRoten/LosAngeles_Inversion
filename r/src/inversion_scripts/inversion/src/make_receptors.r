...
master_times_all <- seq(from = ISOdatetime(y1, m1, d1, h1, mn1, 0, tz = "UTC"),
                        to = ISOdatetime(y2, m2, d2, h2, mn2, 0, tz = "UTC"),
                        by = obs_t_res)  # obs_t_res defined in config.r
...
for (ii in 1:length(sites)) {
  ...
  foot_times <- as.POSIXct(strptime(substr(foot_files, 1, 12),
                                    format = "%Y%m%d%H%M",
                                    tz = "UTC"))
  ...
  isubset_foots <- which(sapply(times_file_fmt,
                                FUN = function(x) any(grepl(x, foot_files))))
  ...
  if (length(iOverlap) == 0) {print(ii); next}
...
if (aggregate_obs) {
  ...
  isites <- lapply(paste0(sites, '/'), FUN = function(x) grep(x, recep_list))
