...
library(ncdf4); library(data.table); library(lubridate); library(raster)
...
isites <- lapply(paste0(sites, '/'),
                 FUN = function(x) grep(x, receptors))
...
# which.min added by D. Roten 6/4/2021
iDomain <- apply(lonlat_domain,
                 FUN = function(x)
                   which.min(sqrt((lonlat_foot[, 1] - x[1])^2 +
                                    (lonlat_foot[, 2] - x[2])^2)),
                 MARGIN = 1)
...
if (make_H_outer) {
  ...
  #nc_lon/lat and in_lon/lat may not match exactly...
  #D. Roten added which.min
  ifoot_complete <- array(1:nrow(lonlat_foot), dim = c(nlon, nlat))
  in_lon <- unique(lonlat_outer[, 1])
  in_lat <- unique(lonlat_outer[, 2])
  imin_lat <- which.min(abs(nc_lat - min(in_lat)))[1]
  imax_lat <- which.min(abs(nc_lat - max(in_lat)))[1]
  imin_lon <- which.min(abs(nc_lon - min(in_lon)))[1]
  imax_lon <- which.min(abs(nc_lon - max(in_lon)))[1]
...
for (ii in 1:nobs) {
    ...
    # Don't break into bins if using satellite data
    if(!file.exists('X_config.RData'))
      times_cut <- as.POSIXct(cut(nc_time, breaks = time_bins), tz = "UTC")
    if(file.exists('X_config.RData'))
      times_cut <- as.POSIXct(nc_time, tz = 'UTC')
  ...
  for (jj in 1:nbins) {
    ...
    if (make_H_outer) {
      ...
      Hsave_outer <- cbind(rep(ii, length(inonzero_outer)),
                           inonzero_outer, Hvec_nonzero_outer)  #ii is obs index
...
y1 <- obs_year_start; y2 <- obs_year_end
m1 <- obs_month_start; m2 <- obs_month_end
d1 <- obs_day_start; d2 <- obs_day_end
h1 <- obs_hour_start; h2 <- obs_hour_end
mn1 <- obs_min_start; mn2 <- obs_min_end

# prepare to aggregate footprint files based on day
time_bins_daily <- seq(from = ISOdatetime(y1, m1, d1, h1, mn1, 0, tz = "UTC"),
                       to = ISOdatetime(y2, m2, d2, h2, mn2, 0, tz = "UTC") + 3600,
                       by = 24 * 3600)

if(!file.exists('X_config.RData'))
  times_cut_day <- as.POSIXct(cut(recep_times, breaks = time_bins_daily),
                              tz = "UTC")
if(file.exists('X_config.RData'))
  times_cut_day <- unique(time_bins_daily)

#read in receptor aggregation file:
#(aggregated file may not exist!)
agg.filepath <- paste0(out_path, "receptors_aggr.rds")
if(file.exists(agg.filepath)) {
...
for (ii in 1:ntimes) {
  ...
  if (make_H_outer) {
    ...
    Hsave_outer <- as.matrix(Hi_outer)
  } #end if make_H_outer
