...
} else if(file.exists('X_config.RData')) {
  # don't break up the times into bins here
  times_cut_all <- as.POSIXct(nc_time, tz = 'UTC')
}
...
iDomain <-
  apply(lonlat_domain,
        FUN = function(x) which((lonlat_prior[, 1] == x[1]) &
                                  (lonlat_prior[, 2] == x[2])), MARGIN = 1)

iDomain <- 
  apply(lonlat_domain,
        FUN = function(x) which.min(sqrt((lonlat_prior[,1] - x[1])^2 +
                                           (lonlat_prior[,2] - x[2])^2)),
        MARGIN = 1)
