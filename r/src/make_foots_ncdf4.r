make_foots_ncdf4 <- function(footprint.list = NULL, averaged.timesteps = NULL,
  ...
  for(i in 1:length(footprint.list)) {
    ...
    layer.names.pos <- as.POSIXct(layer.names, tz = 'UTC',
                                  format = '%Y.%m.%d.%H.%M.%S')
    ...
    new.names <- floor(new.names)
    names(cropped.footprint) <- as.character(new.names)
    
    ######################################
    ##### Work to save the footprint #####
    ######################################
    # make origin consistent
    raster::origin(cropped.footprint) <- c(0,0)
    ...
    #This default time value may be incorrect.
    #It is corrected in future steps
