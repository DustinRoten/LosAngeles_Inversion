make_prior.outer.truth.v4_ncdf4 <- function(site = NULL, odiac.path = NULL,
  ...
  nc_emiss <- nc_create(nc_filename, truth_vars)
  ncvar_put(nc_emiss, truth_emiss_var, truth_values,
  ...
  ncvar_put(nc_prior, prior_emiss_var, prior_values,
            count = c(nlon, nlat, ntime))
  nc_close(nc_prior)
  #####################################
  #####################################
  ...
  ##### Save the Sectors #####
  ############################
  cat(paste0('Generating category emissions...', '\n'))
  make_vulcan.sectors(vulcan_emiss = vulcan.path,
                      sector_df = new.vulcan.sector_df,
                      date.times = times, ref.raster = ODIAC[[1]],
                      output.path = obs.dir)
  ############################
  ############################
  ############################
