prepare.parallelization <- function(homedir, input.extension, workdir, footprint.dirs,
                                    odiac.path, odiac.background.path, odiac.downscaling, vulcan.path,
                                    vulcan.sector.path, smurf.path, use.year, carma.path, edgar.path,
                                    edgar.downscaling, edgar.sector.path, output.directory, site, xmin, xmax,
                                    ymin, ymax, emissions.category, api.key, included.errors, prior.emissions,
                                    truth.emissions, background.emissions, prior.uncertainty, bio.emissions,
                                    prior.lonlat, background.lonlat, observation.values, background.values,
                                    obs.error, include_outer, include_bio, lon_res, lat_res, ls, lt,
                                    aggregation.factor, flux_units, case, bias, ignore.lps,
                                    TEST, test.which) {
  ...
  if(TEST) {
    out.dirs <- out.dirs[test.which]
    footprint.dirs <- footprint.dirs[test.which]
  }
  
  #generate directory paths for the new directories
