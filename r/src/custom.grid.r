custom.grid <- function(lons, lats, values, layers = 1, resolution = 0.05) {
    ...
    for(j in 1:(length(break.lons)-1)) {
      for(i in 1:(length(break.lats)-1)) {
        ...
        if(nrow(tmp_df) != 0) {
          ...
          if(nrow(tmp_df) >= 5) {
            std.err <- sd(tmp_df$values)/sqrt(nrow(tmp_df))
          } else if(nrow(tmp_df) < 5) std.err <- NA
        } else if(nrow(tmp_df) == 0) {
          tmp.lon <- (break.lons[j] + break.lons[j+1])/2
          tmp.lat <- (break.lats[i] + break.lats[i+1])/2
          tmp.val <- 0; std.err <- NA
        }
        ...
        agg_dataframe <- rbind(agg_dataframe,
                               data.frame(lons = tmp.lon,
                                          ...
                                          values = tmp.val,
                                          layers = list.layers[k],
                                          cell.number = num.cell,
                                          num_of_soundings = nrow(tmp_df),
                                          std.err = std.err))
