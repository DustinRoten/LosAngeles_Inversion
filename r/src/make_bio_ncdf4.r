...
make_bio_ncdf4 <- function(SMUrF.path = NULL, SMUrF.output = NULL,
                           times = NULL, match.year = NA,
                           match.raster = NULL, bio.output = NULL) {
  ...
  for(i in 1:length(SMUrF.files)) {
    ...
    tmp.raster <- brick(SMUrF.files[i], varname = SMUrF.output)
  ...
  for(i in 1:length(times)) {
    ...
    smurf.crs <- projection(smurf.raster)
    ...
    smurf.raster.layer <-
      projectRaster(from = smurf.raster.layer, to = match.raster,
                    crs = smurf.crs, method = 'bilinear')
