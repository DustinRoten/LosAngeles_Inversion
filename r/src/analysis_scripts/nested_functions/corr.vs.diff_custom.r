corr.vs.diff_custom <- function(custom.path = NULL, inversion.out.ext = 'inversion/out',
                                obs.error = 0, output.path = 'Out/Custom_Results') {
  ...
  disp.values$scenario[disp.values$scenario != 'Custom_F1'] <-
    'Unmodified'
  disp.values$scenario[disp.values$scenario == 'Custom_F1'] <-
    'Modified'
  disp.values$scenario <- factor(disp.values$scenario,
                                 levels = c('Unmodified', 'Modified'))
  
  lm.data$scenario[lm.data$scenario != 'Custom_F1'] <-
    'Unmodified'
  lm.data$scenario[lm.data$scenario == 'Custom_F1'] <-
    'Modified'
  lm.data$scenario <- factor(lm.data$scenario,
                             levels = c('Unmodified', 'Modified'))
  ...
  gridded.SAMs$layers[gridded.SAMs$layers != 'Custom_F1'] <-
    'Unmodified'
  gridded.SAMs$layers[gridded.SAMs$layers == 'Custom_F1'] <-
    'Modified'
  gridded.SAMs$layers <- factor(gridded.SAMs$layers,
                                levels = c('Unmodified', 'Modified'))
  
  xco2.custom.plot <- ggmap(gg.map.large) +
  ...
  combo.plot <- Corr.Vs.Diff.plot/xco2.custom.plot
  ggsave(combo.plot, device = 'jpg',
         height = 7, width = 6, units = 'in',
         filename = file.path(output.path, 'XCO2_Custom.jpg'))
}
